...
class ApolloUnifiedProtectionEngine extends EventEmitter {
    ...
    async detectLivingOffTheLand() {
        ...
        const suspiciousTools = ['powershell.exe', 'wmic.exe', 'certutil.exe', 'bitsadmin.exe'];
        ...
        try {
            ...
            for (const tool of suspiciousTools) {
                const toolProcesses = processes.filter(line =>
                ...
                if (toolProcesses.length > 0) {
                    this.handleDetectedThreats([{
                        ...
                        reason: `Detected running instance of ${tool}`
                    }], 'Process Monitor');
                }
            }
        } catch (error) {
